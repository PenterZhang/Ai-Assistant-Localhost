  ...
  setInterval(async () => {
    ...
    try {
      ...
      if (msgs.length > 0) {
        ...
        for (const m of msgs) {
          console.log(`  ROWID=${m.rowid} from_me=${m.is_from_me} sender=${m.sender} text=${m.text.slice(0, 30)}`);
        }
      }

      for (const m of msgs) {
        state.lastRowId = Math.max(state.lastRowId, m.rowid);
        if (m.is_from_me) { console.log(`[iMessage] 跳过: 自己发的消息`); continue; }
        if (!m.text.trim()) { console.log(`[iMessage] 跳过: 空消息`); continue; }
        ...
        console.log(`[iMessage] 查找联系人 ${m.sender}: 找到 ${contacts.length} 条`);
        ...
        console.log(`[iMessage] 处理: ${m.sender} → ${m.text.slice(0, 30)}`);
...
fastify.post("/api/imessage/contacts", async (req) => {
  ...
  dbRun("INSERT OR REPLACE INTO imessage_contacts (handle_id,name,auto_reply,model,trigger_mode,created_at) VALUES (?,?,?,?,?,?)",
    [handle_id, name || "", 1, "mimo", trigger_mode || "always", Date.now() / 1000]);
  return { ok: true };
});
