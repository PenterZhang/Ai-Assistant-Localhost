...
async function* streamMimo(messages: Msg[]): AsyncGenerator<string> {
  ...
  const resp = await fetch(`${mc.base_url}/chat/completions`, {
    method: "POST",
    headers: { Authorization: `Bearer ${mc.api_key}`, "Content-Type": "application/json" },
    body: JSON.stringify({
      model: mc.model_id, messages, stream: true,
      temperature: mc.temperature ?? 0.7, max_tokens: mc.max_tokens ?? 4096,
    }),
  });
  if (!resp.ok) {
...
async function* streamOllama(messages: Msg[]): AsyncGenerator<string> {
  ...
  if (!ollamaModel) { yield "\n\n**未配置 Ollama 模型**"; return; }
  const resp = await fetch(`${ollamaModel.base_url}/api/chat`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      model: ollamaModel.model_id, messages, stream: true,
      options: { temperature: ollamaModel.temperature ?? 0.7, num_predict: ollamaModel.max_tokens ?? 4096 },
    }),
  });
  if (!resp.ok) { yield `\n\n**Ollama Error (${resp.status})**`; return; }
  const dec = new TextDecoder();
  let buf = "";
  for await (const chunk of resp.body!) {
    buf += dec.decode(chunk, { stream: true });
    ...
    for (const line of lines) {
      if (!line) continue;
      try { const c = JSON.parse(line).message?.content; if (c) yield c; } catch {}
    }
  }
}
...
async function generateAndSend(
  ...
  if (existing.length) {
    ...
    dbRun(
      ...
      [sid, `iMessage: ${contact.name || targetHandle}`, contact.model, "imessage", targetHandle, n, n]
    );
  }
...
function startPoller() {
  ...
  async function poll() {
    try {
      ...
      heartbeat++;
      if (heartbeat % 10 === 0) {
        console.log(`[iMessage] 心跳: 轮询正常 (ROWID=${state.lastRowId})`);
      }
      ...
      for (const m of msgs) {
        ...
        if (m.is_from_me) {
          ...
          if (!contact) {
            console.log(`[iMessage] /ai 跳过: ${targetHandle} 不在联系人列表`);
            continue;
          }

          console.log(`[iMessage] 自己触发 /ai → ${targetHandle}: ${prompt.slice(0, 50)}`);

          try {
            await generateAndSend(targetHandle, prompt, contact);
          } catch (e) {
            console.error(`[iMessage] /ai 处理失败:`, (e as Error).message);
          }
          continue;
        }
        ...
        console.log(`[iMessage] 处理: ${m.sender} → ${m.text.slice(0, 50)}`);
        ...
        try {
          ...
          console.error(`[iMessage] 自动回复失败:`, (e as Error).message);
        }
      }
    } catch (e) {
  ...
  try {
    ...
    if (existing.length) {
      ...
      dbRun("INSERT INTO sessions (id,title,model,source,imessage_handle,created_at,updated_at) VALUES (?,?,?,?,?,?,?)",
        [sid, `iMessage: ${contact.name || handle}`, contact.model, "imessage", handle, now, now]);
    }
...
fastify.post("/api/sessions", async (req) => {
  ...
  dbRun("INSERT INTO sessions (id,title,model,source,created_at,updated_at) VALUES (?,?,?,?,?,?)",
    [id, "新对话", m, "web", now, now]);
  return { id, title: "新对话", model: m, source: "web", created_at: now, updated_at: now };
});
...
fastify.post("/api/chat", async (req, reply) => {
  const { session_id, message, model } = req.body as { session_id: string; message: string; model?: string };
  ...
  try {
    ...
    reply.raw.write(`data: ${JSON.stringify({ content: `[Error] ${(e as Error).message}` })}\n\n`);
  }

  dbRun("INSERT INTO messages (id,session_id,role,content,model,created_at) VALUES (?,?,?,?,?,?)",
    [crypto.randomUUID(), session_id, "assistant", full, modelId, Date.now() / 1000]);
  ...
  if (n === 1 && !full.startsWith("\n\n**")) {
    dbRun("UPDATE sessions SET title = ? WHERE id = ?", [full.slice(0, 40) + (full.length > 40 ? "…" : ""), session_id]);
  }
...
fastify.post("/api/imessage/contacts", async (req) => {
  const { handle_id, name, trigger_mode } = req.body as { handle_id: string; name?: string; trigger_mode?: string };
  dbRun(
    "INSERT OR REPLACE INTO imessage_contacts (handle_id, name, auto_reply, model, trigger_mode, created_at) VALUES (?, ?, ?, ?, ?, ?)",
    [handle_id, name || "", 1, CFG.default_model, trigger_mode || "always", Date.now() / 1000]
  );
  return { ok: true };
});
...
fastify.post("/api/sleep/toggle", async () => {
  ...
  if (caffeinate && !caffeinate.killed) { caffeinate.kill(); caffeinate = null; return { preventing: false }; }
...
export async function startServer() {
  try {
    ...
    if (!isProd) console.log(`[Server] UI:   http://localhost:5173  ← 打开这个`);
    startPoller();
  } catch (err: any) {
